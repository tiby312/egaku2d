    ...
    events_loop.run(move |event, _, control_flow| match event {
        ...
        Event::EventsCleared => {
            if timer.is_ready() {
                ...
                {
                    let mut k = sys.circles([0., 1., 1., 0.1],100.0);
                ...
                {
                    let mut k = sys.squares([1., 0., 1., 0.1],100.0);
                    for x in (0..1000).step_by(100) {
                        for y in (0..1000).step_by(100) {
                            k.add(vec2(x as f32, y as f32));
                        }
                ...
                sys.lines([0., 1.0, 0., 0.3],10.0)
                    ...
                    .add(vec2(700., 500.), vec2(100., 100.))
                    .draw();

                sys.lines([1., 1., 0.2, 0.2],10.)
                    .add(vec2(50., 500.), vec2(500., 50.))
                    .draw();

                sys.arrows([0.0,1.0,0.1,0.5],5.0).add(vec2(40.,40.),vec2(40.,200.)).draw();
