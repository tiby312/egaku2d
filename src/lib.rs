...
impl FullScreenSystem{
    pub fn new(events_loop:&glutin::event_loop::EventLoop<()>)->Self{
        ...
        FullScreenSystem{windowed_context,inner:MySys::new(game_world),view_port_set:false}
    }
    ...
    pub fn set_viewport_from_width(&mut self,width:f32){
        ...
        self.inner.set_viewport(dim.x,rect(0.0,width,0.0,height));
...
impl WindowedSystem{
	...
	pub fn new(dim:Vec2<f32>,events_loop:&glutin::event_loop::EventLoop<()>)->WindowedSystem{
        ...
        WindowedSystem{windowed_context,inner:MySys::new(game_world)}
	}
